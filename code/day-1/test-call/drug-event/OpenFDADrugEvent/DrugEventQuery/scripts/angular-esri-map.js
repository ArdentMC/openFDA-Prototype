...
(function (angular) {
    ...
    require([
        ...
        function(Map, GraphicsLayer, FeatureLayer, arcgisUtils, PictureMarkerSymbol, SimpleRenderer, Graphic, esriConfig, Print, Extent, Point) {
                ...
                return {
                    ...
                    compile: function($element, $attrs) {
                        ...
                        return function(scope, element, attrs, controller) {
                            scope.$parent.getMapScope = function() { return scope; }
                            scope.$parent.mapInit = controller.init;
                            scope.$parent['mapController'] = controller;
                            //  controller.init();
                        };
                    },
                    ...
                    controller: function($scope, $element, $attrs) {
                            ...
                            mapDeferred.promise.then(function(map) {
                                ...
                                map.on('extent-change', function(e) {
                                    ...
                                    $scope.$apply(function() {
                                        ...
                                        $scope.$parent.map.center = $scope.center;
                                        $scope.$parent.map.zoom = $scope.zoom;
                        ...
                        this.getLayer = function (layerid) {
                            return self.getMap().then(function (map) {
                                return map.getLayer(layerid);
                            });
                        };

                        this.zoomToConus = function () {
                            var conusExtent = new Extent({ xmin: -15826867.9704356, ymin: 2236172.27375974, xmax: -4878557.31711327, ymax: 6922679.3518046, spatialReference: { wkid: 102100 } });
                            self.getMap().then(function(map) {
                                map.setExtent(conusExtent);
                            });

                        };

                        this.addPictureMarker = function (layerId, markerSymbol) {
                            self.getMap().then(function (map) {
                                var graphicLayer = map.getLayer(layerId);
                                if (!graphicLayer) {
                                    graphicLayer = map.addLayer(new GraphicsLayer({ id: layerId }));
                                }
                                graphicLayer.add(new Graphic(point, markerSymbol));
                            });
                        };

                        this.createPictureMarker = function(imageUrl, width, height) {
                            return new PictureMarkerSymbol(imageUrl, width, height);
                        }

                        this.createLayer = function(layerType, layerId) {
                            var layer = null;
                            switch(layerType) {
                                case 'GraphicsLayer':
                        ...
                        this.createPoint = function(lat, lon) {
                            return new Point(lon, lat, new SpatialReference({ "wkid": 4326 }));
                        }

                        this.createGraphic = function (point, marker, attributes, infoTemplate) {
                            return attributes ? new Graphic(point, marker, attributes, infoTemplate) : ( marker ? new Graphic(point, marker) : new Graphic(point));
                        }
                    }
                };
            });
